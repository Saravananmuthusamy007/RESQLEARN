...
import { Lock, Unlock, PlayCircle, CheckCircle, Award, Lightbulb } from 'lucide-react';
...
const LevelList = () => {
  ...
  const [selectedSolutionLevelId, setSelectedSolutionLevelId] = useState(null);
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                {isUnlocked ? (
                  <div className="grid grid-cols-5 gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedSolutionLevelId(level.order);
                      }}
                      title="View Practical Solution Guide"
                      className="col-span-2 flex items-center justify-center px-2 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 font-bold rounded-xl text-xs transition"
                    >
                      <Lightbulb className="w-3.5 h-3.5 mr-1 text-amber-600 fill-amber-600" /> Solution
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="col-span-3 flex items-center justify-center px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs transition shadow"
                    >
                      <PlayCircle className="w-4 h-4 mr-1.5" /> Start Level
                    </button>
                  </div>
                ) : (
                  <button
                    disabled
                    className="w-full flex items-center justify-center px-4 py-2.5 bg-gray-200 text-gray-500 font-medium rounded-xl text-xs cursor-not-allowed"
                  >
      ...
      {/* Global Solution Guide Modal */}
      {selectedSolutionLevelId && (
        <InteractiveSolutionGuide
          levelId={selectedSolutionLevelId}
          isOpen={!!selectedSolutionLevelId}
          onClose={() => setSelectedSolutionLevelId(null)}
        />
      )}
    </div>
  );
